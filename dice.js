...
(function(dice) {
    ...
    this.dice_box.prototype.reinit = function(container, dimentions) {
        ...
        this.camera = new THREE.PerspectiveCamera(12, this.cw / this.ch, 1, this.wh * 2);
        this.camera.position.x = 0;
		this.camera.position.y = -4000;
		this.camera.position.z = this.wh- 100;
		
		this.camera.lookAt(new THREE.Vector3(0,-40,0));

        var mw = Math.max(this.w, this.h);
        if (this.light) this.scene.remove(this.light);
        this.light = new THREE.SpotLight(that.spot_light_color, 4.5);
        this.light.position.set(-mw - 15.8, mw + 1.8, mw * 2);
        this.light.target.position.set(0, 0, 0);
        this.light.distance = mw * 15;
        ...
        this.desk = new THREE.Mesh(new THREE.PlaneGeometry(this.w * 300, this.h * 300, 1, 1), 
    ...
    this.dice_box.prototype.bind_throw = function(button, notation_getter, before_roll, after_roll) {
        var box = this;
        $t.bind(button, ['mouseup', 'touchend', 'click'], function(ev) {
            ev.stopPropagation();
